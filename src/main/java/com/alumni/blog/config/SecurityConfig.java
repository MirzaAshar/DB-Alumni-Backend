...
public class SecurityConfig {
    ...
    public SecurityFilterChain securityFilterChain(HttpSecurity http) throws Exception {
        http
                ...
                .authorizeHttpRequests(auth -> auth
                        .requestMatchers("/api/auth/**", "/api/auth/login", "/error", "/api/categories/", "/api/user/{userId}/category/{categoryId}/posts", "/api/users/{userID}", "/api/posts", "/api/**").permitAll() // Allow unauthenticated access to auth endpoints
    ...
    public CorsFilter corsFilter() {
        ...
        config.setAllowedOrigins(List.of("https://db-project-zeta.vercel.app","http://localhost:3000")); // Adjust allowed origins as necessary
